...
class BabyLMDatasetBuilder:
    ...
    def __init__(self, dataset_config: DatasetConfig, output_dir: Path = Path("./babylm_datasets")):
    ...
    def add_documents_from_directory(self, 
        ...
        for text_file in texts_dir.glob("*.txt"):
            ...
            doc_config = DocumentConfig(
                category=doc_overrides.get('category', default_document_config.category),
                data_source=doc_overrides.get('data_source', default_document_config.data_source),
                script=doc_overrides.get('script', default_document_config.script),
                age_estimate=doc_overrides.get('age_estimate', default_document_config.age_estimate),
                ...
                source_url=doc_overrides.get('source_url', default_document_config.source_url),
                source_identifier=doc_overrides.get('source_identifier', default_document_config.source_identifier)
            )
            
            # Extract additional metadata (non-config fields)
            additional_metadata = {k: v for k, v in doc_overrides.items() 
                                 if k not in ['category', 'data_source', 'script', 
                                            'age_estimate', 'license', 'misc', 
    ...
    def create_dataset_table(self) -> pd.DataFrame:
        ...
        parquet_path = self.output_dir / f"{self.dataset_config.dataset_name}_dataset.parquet"
