...
BASE_URL = "https://object.pouta.csc.fi/OPUS-OpenSubtitles/v2024/xml/{lang}.zip"
...
class OpenSubtitlesProcessor:
    ...
    def download_zip(self, dest: Path) -> Path:
        ...
        with requests.get(url, stream=True) as r:
            ...
            bar = tqdm(
                total=total, unit="B", unit_scale=True, desc=f"⬇ {self.lang_code}.zip"
            )
        ...
        try:
            ...
            for idx, part in enumerate(parts):
                if (
                    part == "xml"
                    and idx + 1 < len(parts)
                    and parts[idx + 1] == self.lang_code
                ):
            ...
            if preprocessor:
                ...
                for sentence in sentences:
                    if sentence == "":  # Preserve empty lines as paragraph breaks
                        ...
                        cleaned = re.sub(
                            r"\s+", " ", cleaned
                        )  # Only normalize spaces, not newlines
                        if cleaned:
                ...
                for i, sent in enumerate(processed_sentences):
                    if sent == "":
                        ...
                        if (
                            i < len(processed_sentences) - 1
                            and processed_sentences[i + 1] != ""
                        ):
                            processed_text.append(
                                "\n"
    ...
    def process_zip_in_batches(
        ...
        batch_size: int = 50,
        preprocessor: Optional["BasePreprocessor"] = None,
        enable_imdb_filtering: bool = True,
    ) -> pd.DataFrame:
        ...
        if enable_imdb_filtering:
            try:
                ...
                cursor = conn.cursor()
                print(
                    f"Connected to IMDB database. Filtering enabled with forbidden genres: {self.forbidden_genres}"
                )
            except Exception as e:
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as zip_file:
                ...
                for i in tqdm(
                    range(0, len(all_xml_members), batch_size),
                    ...
                    for member_name in batch_members:
                        year, folder_id, file_id = self.extract_file_metadata(
                            member_name
                        )
                        ...
                        try:
                            with (
                                zip_file.open(member_name) as source,
                            ...
                            success = self.process_xml_to_text(
                                extracted_path, text_output_path, preprocessor
                            )
                ...
                if enable_imdb_filtering:
                    ...
                    print(
                        f"Retention rate: {(processed_count / total_files) * 100:.1f}%"
                    )
    ...
    def process_language(
        ...
        metadata_df = self.process_zip_in_batches(
            zip_dest, batch_size, preprocessor, enable_imdb_filtering
        )
